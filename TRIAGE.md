...
### 🔧 PHASE 8 (2026-06-08) — blank screen after exit (viii regressed) → v0.2.57
You hit a blank terminal after quitting. Captured the real exit byte stream in a PTY and found the cause:
- ☑ **Ink wipes the scrollback every frame.** Ink's ansi-escapes `clearTerminal` = `\x1b[2J\x1b[3J\x1b[H`, fired `if (outputHeight >= stdout.rows)`. The `\x1b[3J` ERASES SCROLLBACK. The fullscreen frame fills the screen, so Ink ran it every render, wiping scrollback continuously; leaving the alt-screen on exit then showed blank. NOT our restore code (that was already correct since v0.2.46). Fix: (1) App.tsx `transcriptHeight = rows - HEADER - footer - 1` (under-fill one row → Ink stays on incremental-erase, no clearTerminal); (2) cli.tsx wraps `stdout.write` in fullscreen to strip any `\x1b[3J` — bulletproof scrollback guard. PTY-verified: 3J count 0.
- ☑ **mouse-restore consistency**: `runInteractive` (/account login spawn) toggled mouse on bare `GEARBOX_MOUSE`, not `fullscreen` — so inline could leave the mouse grabbed after exit (with v0.2.56's `mouse = fullscreen`). Now both toggles gated on fullscreen.
- ☑ paste-coalescer timer cleared on unmount (hygiene).
- Full regression audit (v0.2.51→now: mouse symmetry, height math, paste, palette arrows, hullSelection, theme) found NO other live regressions. The I-A markerless-paste coalescer (TRIAGE earlier marked "deferred") DID ship in v0.2.53 and you confirmed paste works — treat I-A as DONE.
- **Lesson: a blank-screen-after-exit in an alt-screen TUI is almost always `\x1b[3J` (erase-scrollback) leaking from the render lib when the frame == terminal height. Capture the real PTY byte stream (`script -q`) rather than theorizing.**
