...
import { applyVerificationPatch, verifyCompactionSummary } from "../src/context/compact-verify.ts";

test("parseCompactionSummary parses JSON into a rendered durable note", () => {
  const parsed = parseCompactionSummary(JSON.stringify({
    goals: ["fix auth"],
    decisions: ["keep retry bounded"],
    files: [{ path: "src/accounts/health.ts", change: "classified expired tokens" }],
    ...
    topics: [{ title: "auth", notes: ["token expiry"], files: ["src/accounts/health.ts"] }],
  }));
...
test("verifyCompactionSummary patches missing mandatory anchors", () => {
  const old: ModelMessage[] = [
    ...
    { role: "assistant", content: [{ type: "tool-call", toolCallId: "1", toolName: "edit_file", input: { path: "src/accounts/health.ts" } }] as any },
    { role: "assistant", content: [{ type: "tool-call", toolCallId: "2", toolName: "run_shell", input: { command: "bun test test/health.test.ts" } }] as any },
    { role: "tool", content: [{ type: "tool-result", toolCallId: "2", toolName: "run_shell", output: { type: "text", value: "failed: token expired error" } }] as any },
  ];
  ...
  expect(verification.missingCommands).toContain("bun test test/health.test.ts");
