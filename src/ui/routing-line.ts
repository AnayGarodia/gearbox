...
export function formatTurnCost(costUSD: number, kind: RouteKind): string {
...
export function buildRoutingLine(input: RoutingLineInput): RoutingLine {
  const { surprising, reason } = classifySurprise(input);
  return {
    model: input.model,
    provider: input.provider,
    costText: formatTurnCost(input.costUSD, input.kind),
    surprising,
    reason,
  };
}
