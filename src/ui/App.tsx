...
import { friendlyError, firstPath, uniq, formatDuration, shortFailure, backendKeyOf, codexCliModels, effortDescription, previewLang, filePreview, isWriteLikeTool, retrievalUseMeta, CLAUDE_CLI_EFFORTS, type CliModelChoice } from "./app-helpers.ts";
...
const searchRef = useRef<{ q: string; idx: number } | null>(null);
  ...
  // The transcript as a flat styled-line buffer. The content column is a
  // centered reading measure: ~82% of the usable width, never wider than a
  // comfortable line (≤110 cols) — a transcript that reads like a document, not
  // a log dump spanning an ultrawide. The margin is baked into the lines (not a
