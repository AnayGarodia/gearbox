...
export function buildCliArgs(binary: string, prompt: string, opts: { sessionId?: string; autoApprove?: boolean; readOnly?: boolean; modelId?: string; effort?: string; addDirs?: string[]; repoRoot?: string; bridge?: boolean } = {}): string[] {
...
export async function runCliTask(opts: {
  ...
  const args = buildCliArgs(binary, prompt, { sessionId: opts.sessionId, autoApprove: opts.autoApprove, readOnly: opts.readOnly, modelId: opts.modelId, effort: opts.effort, addDirs, repoRoot: opts.cwd ?? process.cwd(), bridge });
  const state = newState();
  ...
  let proc: Proc;
  try {
    proc = spawnProc([binary, ...args], { stdin: bridge ? "pipe" : "ignore", stdout: "pipe", stderr: "pipe", cwd: opts.cwd ?? process.cwd(), env: subscriptionEnv(binary, opts.profile) });
  } catch (e: any) {
