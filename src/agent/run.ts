...
export async function runTask(opts: {
  ...
  const delegateTools = depth === 0 && !plan ? makeDelegateTools({ onEvent, signal, run: subRunner, pinnedModelId: opts.pinnedModelId, root: opts.root, onBackground: opts.onBackground }) : undefined;
